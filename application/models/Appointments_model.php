...
class Appointments_model extends EA_Model
{
    ...
    public function export($data)
    {
        ...
        // $d = $this->db->get_where('appointments', ['start_datetime >' => '2024-08-11'])->join('users AS providers', 'providers.id = appointments.id_users_provider', 'inner')
        //     ->join('users AS customers', 'customers.id = appointments.id_users_customer', 'left')->result_array();

        $d = $this->db
            ->select('appointments.*, 
                  services.name AS service_name, 
                  providers.first_name AS doctor_first_name, 
                  providers.last_name AS doctor_last_name, 
                  customers.first_name AS patient_first_name,
                  customers.last_name AS patient_last_name,
                  customers.custom_field_2 AS patient_file_number,
                  customers.custom_field_1 AS patient_age, 
                  customers.custom_field_3 AS patient_nationality,
                  customers.phone_number AS patient_phone_number')
            ->from('appointments')
            ->join('services', 'services.id = appointments.id_services')
            ->join('users AS providers', 'providers.id = appointments.id_users_provider')
            ->join('users AS customers', 'customers.id = appointments.id_users_customer')
            ->where('appointments.start_datetime >=', $data['from'] . ' 00:00:00')
            ->where('appointments.end_datetime <=', $data['to'] . ' 23:59:59')
            ->get()
            ->result_array();
        return $d;
    }
